...
UVIPCMessage* MakeUVIPCMsg(YUVRawDataI420* data_, unsigned long long recv_handle, unsigned int type)
{
	...
	else
	{
		...
		//copy Y data
		head_ptr->y_offset = 0;
		memcpy(data_buf, data_->GetYBuffer(), (data_->GetStreamWidth() * data_->GetStreamHeight()));  //checked safe
		//copy U data
		head_ptr->u_offset = (data_->GetStreamWidth() * data_->GetStreamHeight());
		memcpy(data_buf + head_ptr->u_offset, data_->GetUBuffer(), (data_->GetStreamWidth() * data_->GetStreamHeight()) / 4);   //checked safe
		//copy V data
		head_ptr->v_offset = head_ptr->u_offset + (data_->GetStreamWidth() * data_->GetStreamHeight()) / 4;
		memcpy(data_buf + head_ptr->v_offset, data_->GetVBuffer(), (data_->GetStreamWidth() * data_->GetStreamHeight()) / 4);    //checked safe
...
UVIPCMessage* MakeUVIPCMsg(ZoomNodeYUVRawdataI420* data_, unsigned long long recv_handle, unsigned int type)
{
	...
	//copy Y data
	head_ptr->y_offset = 0;
	memcpy(data_buf, data_->ybuff, (data_->streamWidth * data_->streamHeight));       //checked safe
	//copy U data
	head_ptr->u_offset = (data_->streamWidth * data_->streamHeight);
	memcpy(data_buf + head_ptr->u_offset, data_->ubuff, (data_->streamWidth * data_->streamHeight)/4);       //checked safe
	//copy V data
	head_ptr->v_offset = head_ptr->u_offset + (data_->streamWidth * data_->streamHeight)/4;
	memcpy(data_buf + head_ptr->v_offset, data_->vbuff, (data_->streamWidth * data_->streamHeight)/4);       //checked safe
...
UVIPCMessage* MakeUVIPCMsg(ZoomNodeAudioRawdata* data_, unsigned int node_id_, unsigned int type)
{
	...
	//fill buffer
	char* data_buf = buf_ptr + sizeof(AudioRawDataHeader);
	memcpy(data_buf, data_->buff, dataBufferLen);      //checked safe
