...
void ZoomNodeMeetingAnnotationWrap::StartAnnotation(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	do
	{
		...
		case 1:
			...
			break;
		case 2:
			_zn_sdk_view_type = ZNSDK_SEND_SHARE_VIEW;
			break;
		default:
			break;
		}
...
void ZoomNodeMeetingAnnotationWrap::IsAnnotationLegalNoticeAvailable(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	bool bDisable = _g_native_wrap.GetMeetingServiceWrap().GetMeetingAnnotation().IsAnnotationLegalNoticeAvailable();
	v8::Local<v8::Boolean> bret = v8::Boolean::New(isolate, bDisable);
	args.GetReturnValue().Set(bret);
}
void ZoomNodeMeetingAnnotationWrap::GetAnnotationLegalNoticesPrompt(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZoomSTRING zn_prompt = _g_native_wrap.GetMeetingServiceWrap().GetMeetingAnnotation().GetAnnotationLegalNoticesPrompt();
	v8::Local<v8::String> bret = v8::String::NewFromUtf8(isolate, zs2s(zn_prompt).c_str(), v8::NewStringType::kInternalized).ToLocalChecked();
	args.GetReturnValue().Set(bret);
}
void ZoomNodeMeetingAnnotationWrap::GetAnnotationLegalNoticesExplained(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZoomSTRING zn_explained = _g_native_wrap.GetMeetingServiceWrap().GetMeetingAnnotation().GetAnnotationLegalNoticesExplained();
