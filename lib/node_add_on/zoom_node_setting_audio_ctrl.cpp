...
void ZoomNodeSettingAudioCtrlWrap::SetEchoCancellationLevel(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZNSDKError err = ZNSDKERR_SUCCESS;
	do
	{
		com::electron::sdk::proto::SetEchoCancellationLevelParams proto_params;
		if (!SetProtoParam<com::electron::sdk::proto::SetEchoCancellationLevelParams >(args, proto_params))
		{
			err = ZNSDKERR_INVALID_PARAMETER;
			break;
		}
		if (!proto_params.has_level())
		{
			err = ZNSDKERR_INVALID_PARAMETER;
			break;
		}
		ZN_SDK_ECHO_CANCELLATION_LEVEL zn_level = ZN_SDK_ECHO_CANCELLATION_DEFAULT;
		zn_level = (ZN_SDK_ECHO_CANCELLATION_LEVEL)proto_params.level();
		err = _g_native_wrap.GetSettingServiceWrap().GetSettingAudioCtrl().SetEchoCancellationLevel(zn_level);
	} while (false);
	
	v8::Local<v8::Integer> bret = v8::Integer::New(isolate, (int32_t)err);
	args.GetReturnValue().Set(bret);
}
void ZoomNodeSettingAudioCtrlWrap::GetEchoCancellationLevel(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	v8::Isolate* isolate = args.GetIsolate();
	ZN_SDK_ECHO_CANCELLATION_LEVEL zn_level = _g_native_wrap.GetSettingServiceWrap().GetSettingAudioCtrl().GetEchoCancellationLevel();
	v8::Local<v8::Integer> bret = v8::Integer::New(isolate, (int32_t)zn_level);
	args.GetReturnValue().Set(bret);
}
